---
id: 3
title: Integrated RAG Chatbot Completion
stage: green
date: '2025-12-17'
...
labels: [feature, implementation, rag, chatbot, docusaurus, fastapi, completion]
links:
  spec: null
  ticket: null
  adr: null
...
# Integrated RAG Chatbot Completion

## Summary

Complete implementation of the integrated RAG chatbot for the Physical AI & Humanoid Robotics book. This implementation includes both backend and frontend components with full functionality for both RAG mode and selection-only mode, with crash fixes and proper Docusaurus integration.
...
- Proper mounting in Layout component using DOM-based theme detection (no useColorMode hook)
- Session persistence using localStorage
- Responsive design with high z-index values for visibility (z-index: 200000)
- Both RAG mode (with citations) and selection-only mode

### Crash Fix Implementation
- Removed all useColorMode hook usage from custom code
- Implemented DOM-based theme detection using document.documentElement.getAttribute('data-theme')
- Added MutationObserver to detect theme changes dynamically
- Layout component no longer depends on ColorModeProvider context
- Root component remains hook-free passthrough
...
- High Visibility: Floating launcher button with fixed positioning (right: 24px; bottom: 24px)
...
- Proper Docusaurus provider context handling with DOM-based theme detection
- No "useColorMode called outside ColorModeProvider" crashes
...
- Debug badge removed from production UI

## Acceptance Criteria Met
- Chat widget appears on all pages with high visibility
- No crashes related to useColorMode hook
...
- Widget positioning and styling properly implemented

The implementation follows the exact design specification with proper component mounting in the Layout wrapper using DOM-based theme detection to avoid provider context issues, and the widget is now visible with high z-index values ensuring it appears above all other UI elements.